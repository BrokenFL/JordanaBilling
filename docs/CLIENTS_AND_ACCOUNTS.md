...
- **Invoice delivery contact:** The billing relationship editor can create or select a separate delivery contact. For organization payers, the contact is durably linked via `billing_parties.person_id` and does not become a covered client or Bill To. For person payers, the contact's delivery details (name, email, phone) are copied onto the billing-party record as a **deliberate contact-detail override** — the payer's `person_id` is preserved, but the billing name/email/phone fields reflect the selected delivery contact. This is not a separate delivery-recipient link; changing contacts overwrites the prior contact's details on the billing party.
