...
`invoices.invoice_date` is a finalized document snapshot, not a service date or
editable draft date. Finalization derives it from `finalized_at` in
`America/New_York`; correction invoices receive the date of their own
finalization. Existing finalized invoices remain frozen.
