...
The Review -> Finalize confirmation screen embeds the canonical draft PDF from the same-origin `GET /api/invoices/{id}/finalization-preview-pdf` endpoint. The draft invoice editor embeds the same canonical draft PDF family from `GET /api/invoices/{id}/draft-pdf` automatically when a draft is opened, then keeps the edit/action buttons below the PDF preview. The old in-app HTML invoice card is not an approval representation and must not be used to ask Jordana to approve a final invoice layout. The preview request is side-effect free: it does not assign a number, finalize, write `pdf_path` or checksum fields, update status/revision, or create audit records. Temporary insurance/coding preview values are carried by a short-lived in-memory token rather than a blob URL or database mutation.

After confirmation, the workflow finalizes through `POST /api/invoices/{id}/finalize`, receives the stored finalized invoice record, and opens the versioned `final_pdf_url` returned on that record. The Invoices workspace embeds that stored final PDF inline for finalized and void invoices while keeping `Open PDF`, Finder, client-folder, and print actions available below the preview. The UI must not leave the user relying on an in-app HTML invoice card as proof of the finalized document layout; the canonical customer-facing artifact is the PDF served by `GET /api/invoices/{id}/final-pdf`.
