...
- **Long service labels**: the PDF measures each service label and transfers available width from Participants to Service so ordinary full labels stay on one line. If a label is too long for the page, it wraps in full rather than being clipped. In-app invoice and receipt tables keep full labels on one line with a keyboard-accessible horizontal scroll area on narrow screens; print previews reserve more space for Service and explicitly wrap unusually long labels.
...
ReportLab's PDF path imports Pillow (`PIL.Image`) at runtime, and draft packet PDF assembly uses `pypdf`. Pillow and pypdf are explicit production dependencies. The release installer plus installed-app verifier import `PIL`, `pypdf`, and ReportLab so missing PDF dependencies are caught during installation instead of surfacing later as a draft-preview or draft-packet failure.
